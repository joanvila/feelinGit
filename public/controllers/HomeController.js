...
function HomeController($scope, $state, githubService, alchemyService) {
    ...
    $scope.analyzeLink = function() {
        ...
        if (typeof($scope.inputData) === 'undefined') alert('You must fill the github url and the alchemy API key');
        else if (typeof($scope.inputData.url) === 'undefined') alert('You must provide a github repository url');
        else if (typeof($scope.inputData.apiKey) === 'undefined') alert('You must provide an Alchemy API Key. It is very easy to get one');
        else {
            ...
            githubService.getCommits(owner, repo).then(function(commits) {
                ...
                for (var i = 0; i < commits.length; ++i) {
                    ...
                    if (goodCommit) {
                        ...
                        alchemyService.getSentiment(commits[i].commit.message, commits[i].commit.author.name, $scope.inputData.apiKey).then(function(sentiment) {
